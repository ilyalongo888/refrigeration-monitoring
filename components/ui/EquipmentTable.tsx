...
interface EquipmentTableProps {
  headers: { equipment: string; temperature: string; target: string; status: string };
      ...
      <table className="w-full min-w-[460px] border-collapse text-left">
        <thead>
          <tr className="border-b border-line text-[10px] uppercase tracking-wide text-text-secondary">
            ...
            <th className="px-3 py-2.5 font-medium">{headers.target}</th>
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className={i !== rows.length - 1 ? "border-b border-line/70" : ""}>
              ...
              <td className="px-3 py-2.5 font-mono text-[13px] text-text-secondary">{row.target}</td>
